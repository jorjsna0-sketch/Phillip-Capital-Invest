...
export function MobileInvestWizard() {
  ...
  const { t, formatCurrency, formatUsdWithEquivalent, getLocalizedText, currency, language } = useLanguage();
  ...
  const validateStep1 = () => {
    ...
    if (numAmount < portfolio.min_investment) {
      setError(`${language === 'ru' ? 'Минимальная сумма' : 'Minimum amount'}: ${formatUsdWithEquivalent(portfolio.min_investment)}`);
      return false;
    }
    if (numAmount > portfolio.max_investment) {
      setError(`${language === 'ru' ? 'Максимальная сумма' : 'Maximum amount'}: ${formatUsdWithEquivalent(portfolio.max_investment)}`);
      return false;
    }
    if (numAmount > availableBalance) {
      setError(`${language === 'ru' ? 'Недостаточно средств. Доступно' : 'Insufficient funds. Available'}: ${formatUsdWithEquivalent(availableBalance)}`);
      return false;
    }
    if (!duration) {
              ...
              <div className="mobile-card p-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-semibold text-sm">{formatUsdWithEquivalent(availableBalance)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-shrink-0 mobile-balance-card mt-2">
                <div className="relative z-10 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">+{formatUsdWithEquivalent(expectedReturn())}</p>
                  </div>
              ...
              <div className="mobile-card p-3">
                ...
                <div className="space-y-2 mb-3 text-sm">
                  ...
                  <div className="flex justify-between py-1.5">
                    ...
                    <span className="font-bold text-green-600">+{formatUsdWithEquivalent(expectedReturn())}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-shrink-0 bg-gradient-to-br from-primary to-emerald-700 rounded-2xl p-3 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{formatUsdWithEquivalent(parseFloat(amount))}</p>
                  </div>
                  <div className="text-right text-sm">
                    ...
                    <p className="text-white font-medium">+{formatUsdWithEquivalent(expectedReturn())}</p>
                  </div>
                </div>
              </div>
            </div>
